...
/*
 List implementation with a private node class
*/
...
public class List {
    ...
    public int index(){
        // Returns the index of the cursor element in
        //this which which is set to -1 if undefined;
        if (cursor == null)
            index = -1;
        return index;
    }
    ...
    public void insertBefore(int data){
        if (length() == 0 || index < 0 )
            throw new RuntimeException("insertBefore() called on null cursor");
        if( index >= length())
